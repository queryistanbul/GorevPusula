<?php
require_once __DIR__ . '/../src/functions.php';
require_once __DIR__ . '/../src/auth.php';
require_once __DIR__ . '/../src/db.php';

check_auth();
$user = Auth::user();

// Only admin
if ($user['role'] !== 'admin') {
    redirect('index.php');
}

$db = Database::getInstance();
$error = null;
$success = null;

// Handle Post
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    verify_csrf();

    if (isset($_POST['action']) && $_POST['action'] === 'delete') {
        $id = $_POST['department_id'];
        try {
            $db->execute("DELETE FROM departments WHERE id = ?", [$id]);
            $success = "Bölüm başarıyla silindi.";
        } catch (Exception $e) {
            $error = "Bu bölüm silinemez. İlişkili kullanıcılar veya görevler olabilir.";
        }
    } else {
        $name = $_POST['name'];
        $description = $_POST['description'];

        if (empty($name)) {
            $error = "Bölüm adı zorunludur.";
        } else {
            try {
                $db->execute("INSERT INTO departments (name, description) VALUES (?, ?)", [$name, $description]);
                $success = "Bölüm başarıyla eklendi.";
            } catch (Exception $e) {
                $error = "Hata: " . $e->getMessage();
            }
        }
    }
}

$departments = $db->query("SELECT * FROM departments ORDER BY name");

require SRC_DIR . '/partials/header.php';
?>

<div class="page-header">
    <div>
        <h1 class="page-title">Bölüm Yönetimi</h1>
        <p class="page-subtitle">Şirket bölümlerini yönetin.</p>
    </div>
</div>

<?php if ($error): ?>
    <div class="alert-error">
        <?= $error ?>
    </div>
<?php endif; ?>

<?php if ($success): ?>
    <div class="alert-success">
        <?= $success ?>
    </div>
<?php endif; ?>

<div class="form-grid-2">

    <!-- Add Form -->
    <div class="glass glass-card">
        <h3 class="card-title">Yeni Bölüm Ekle</h3>
        <form method="POST">
            <?= csrf_field() ?>
            <input type="hidden" name="action" value="add">

            <div class="form-group">
                <label>Bölüm Adı</label>
                <input type="text" name="name" required placeholder="Örn: İnsan Kaynakları">
            </div>

            <div class="form-group">
                <label>Açıklama</label>
                <textarea name="description" rows="3" placeholder="Bölüm açıklaması..."></textarea>
            </div>

            <button type="submit" class="btn btn-primary btn-block">Bölüm Oluştur</button>
        </form>
    </div>

    <!-- List -->
    <div class="glass glass-card">
        <h3 class="card-title">Bölüm Listesi (
            <?= count($departments) ?>)
        </h3>

        <table class="table">
            <thead>
                <tr>
                    <th>Bölüm Adı</th>
                    <th>Açıklama</th>
                    <th class="text-right">İşlem</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($departments as $d): ?>
                    <tr style="border-bottom: 1px solid var(--glass-border);">
                        <td style="padding: 10px; font-weight: 500;">
                            <?= escape($d['name']) ?>
                        </td>
                        <td style="padding: 10px; color: var(--text-muted);">
                            <?= escape($d['description']) ?>
                        </td>
                        <td style="padding: 10px; text-align: right;">
                            <form method="POST" onsubmit="return confirm('Silmek istediğinize emin misiniz?');"
                                style="display: inline;">
                                <?= csrf_field() ?>
                                <input type="hidden" name="action" value="delete">
                                <input type="hidden" name="department_id" value="<?= $d['id'] ?>">
                                <button type="submit" class="btn-text-danger">Sil</button>
                            </form>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
</div>

<?php require SRC_DIR . '/partials/footer.php'; ?>